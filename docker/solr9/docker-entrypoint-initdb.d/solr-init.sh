...
CKAN_SOLR_SCHEMA_URL=/docker-entrypoint-initdb.d/overwrite.schema.xml
...
#  exit 1
#fi
if [ -e "$CKAN_SOLR_SCHEMA_URL" ]; then
  echo "Found CKAN SOLR schema at PATH: $CKAN_SOLR_SCHEMA_URL. Proceding..."
else 
  echo "Can't find CKAN SOLR schema at PATH: $CKAN_SOLR_SCHEMA_URL. Exiting..."
  exit 1
fi
...
if [ -d "$COREDIR" ]; then
    ...
    cp -v $CKAN_SOLR_SCHEMA_URL /var/solr/data/$CKAN_CORE_NAME/conf/managed-schema
